...
MOX sensors are also sensitive to humidity, but they have a fundamental advantage: heating to 300 to 400°C evaporates adsorbed water from the surface of the sensing layer during every temperature scan cycle. In effect, each temperature step is a self-cleaning mechanism. CNT sensors operate at room temperature and lack this capability entirely.
...
Eight BME688 sensors on a single PCB. Each operates on an individual temperature profile of ten steps. Scan cycle duration: 3 to 5 seconds. Eight sensors, ten steps, four parameters per step, 320 data points per cycle.
...
Preliminary validation on a dataset of 23 individual compounds and 34 binary mixtures (57 classes total): classification accuracy of 91.3% at humidity levels of 20 to 80% RH and ambient temperatures of 5 to 35°C. Performance drops to 84% when ternary mixtures are included, an expected result given the exponential growth of the mixture space.
